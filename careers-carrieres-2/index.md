...
function arrow(direction) {
  if (document.getElementById('header-police').classList.contains('active')) {
    ...
    switch (direction) {
      case 'left':  headernav(leftNav);  break;
      case 'right': headernav(rightNav); break;
    }
...
function renderQuestion() {
  ...
  if (question.details || question.bullets.length > 0) {
    ...
    html += question.bullets.length > 0
      ? `<p>${question.details}</p>`
      : `<p class="mrgn-bttm-0">${question.details}</p>`;
    if (question.bullets.length > 0) {
      html += '<ul>' + question.bullets.map(b => `<li>${b}</li>`).join('') + '</ul>';
    }
    html += '</div>';
  }

  html += `
    <div class="question-legend" id="question-legend-${question.number}">
      <p><strong>${question.question}</strong></p>
  `;
  if (question.questionBullets.length > 0) {
    html += '<ul>' + question.questionBullets.map(b => `<li>${b}</li>`).join('') + '</ul>';
  }
  ...
  document.getElementById(`yes-${question.number}`).addEventListener('change', () => selectAnswer('yes'));
  document.getElementById(`no-${question.number}`).addEventListener('change', () => selectAnswer('no'));
...
function selectAnswer(answer) {
  ...
  const infoSection = document.getElementById(`info-section-${questions[currentQuestionIndex].number}`);
  if (infoSection) {
    infoSection.classList.add('show');
    if (wasNull) announce('Additional information is now available below the answer choices.');
  }
...
const elementOffset = document.querySelector('#quiz-section');
...
function showResults() {
  ...
  if (yesCount === questions.length) {
    ...
    const noItems = answers
      .map((a, i) => a === 'no'
        ? `<li><a href="${questions[i].link}" target="_blank" rel="noopener noreferrer" aria-label="${questions[i].title} - Opens in a new window">${questions[i].title} <span class="sr-only">(opens in new window)</span></a></li>`
        : '')
      .join('');
  ...
  announce(yesCount === questions.length
    ? 'Quiz complete! Congratulations, you appear suited to be an RCMP police officer.'
    : `Quiz complete! You answered yes to ${yesCount} out of ${questions.length} questions. Additional preparation may be needed.`
  );
...
document.addEventListener('keydown', (e) => {
  if (e.target.type !== 'radio') {
    ...
    else if (e.key === 'ArrowLeft' && !prevButton.disabled) prevButton.click();
  }
});
