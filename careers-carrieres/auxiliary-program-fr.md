---
layout: careers
title: Programme des auxiliaires 
date_modified: 2026-07-13
lang: fr
lang_url: auxiliary-program-en.html
breadcrumbs:
  - label: "Carrières à la GRC"
    url: "careers-carrieres/fr.html"
  - label: "Carrières policières"
    url: "careers-carrieres/fr.html"
custom_css: /assets/css/careers.css
---
<nav aria-labelledby="on-this-page">
  <h2 id="on-this-page">Sur cette page</h2>
  <ul>
    <li><a href="#s1">Aperçu du programme </a></li>
    <li><a href="#s2">Qui peut devenir auxiliaire </a></li>
    <li><a href="#s3">Fonctions opérationnelles </a></li>
    <li><a href="#s4">Trois niveaux de service </a></li>
    <li><a href="#s5">Comment poser sa candidature </a></li>
    <li><a href="#s6">Pour en savoir plus </a></li>
  </ul>
</nav>
<section id="s1">
  <h2>Aperçu du programme </h2>
  <p>Créé en 1963, le Programme des auxiliaires vise à appuyer les initiatives de police communautaire et de prévention du crime. Les auxiliaires offrent bénévolement de leur temps pour contribuer à assurer la sécurité des communautés tout en acquérant une expérience enrichissante aux côtés des employés de la GRC. La GRC accorde une grande valeur à l’apport de ces bénévoles et les considère comme une composante essentielle de l’organisation. Le programme est structuré en trois niveaux de service dans l’ensemble des provinces et des territoires.</p>
</section>
<section id="s2">
  <h2>Qui peut devenir auxiliaire </h2>
  <p>Les auxiliaires sont des bénévoles non armés qui ont reçu une formation spécialisée. Les postulants au Programme des auxiliaires doivent&#160;:</p>
  <ul>
    <li>être disposés à consacrer un nombre minimal d'heures par année au programme; </li>
    <li>être disposés à faire partie du programme pendant au moins deux ans; </li>
    <li>détenir un certificat de secourisme général valide et en réanimation cardiorespiratoire; </li>
    <li>pour les postes de niveau&#160;3, avoir réussi le Cours canadien de sécurité dans le maniement des armes à feu et le Cours canadien de sécurité dans le maniement des armes à feu à autorisation restreinte; </li>
    <li>avoir au moins 19&#160;ans; </li>
    <li>être citoyens canadiens ou être résidents permanents; </li>
  </ul>
  <p>Remarque&#160;: Les candidats ayant le statut de résident permanent doivent avoir résidé au Canada au cours des cinq dernières années consécutives. </p>
  <ul>
    <li>avoir de la maturité, être responsables et avoir une bonne réputation; </li>
    <li>avoir terminé avec succès leurs études secondaires ou l'équivalent, ou avoir une expérience de travail équivalente; </li>
    <li>être en mesure d'obtenir une cote de fiabilité de la GRC; </li>
    <li>n'avoir aucun conflit d'intérêts; </li>
    <li>maîtriser le français ou l'anglais; </li>
    <li>pour les postes de niveaux&#160;2 et&#160;3&#160;:
      <ul>
        <li>satisfaire aux normes médicales, dont celles de la vision et de l'ouïe; </li>
        <li>détenir un permis de conduire valide. </li>
      </ul>
    </li>
  </ul>
</section>
<section id="s3">
  <h2>Fonctions opérationnelles </h2>
  <p>Les activités des auxiliaires varient d'une province à l'autre, mais peuvent comprendre&#160;: </p>
  <ul>
    <li>initiatives de prévention de la criminalité et de sensibilisation du public; </li>
    <li>aide en cas de catastrophe; </li>
    <li>contrôle de la circulation; </li>
    <li>patrouilles des services généraux et réponse aux appels de service avec des membres réguliers. </li>
  </ul>
</section>
<section id="s4">
  <h2>Trois niveaux de service </h2>
  <p>Consultez les attentes et les exigences pour chaque niveau de service.</p>
  <h3>Fonctions opérationnelles</h3>
  <dl>
    <dt>Niveau&#160;1</dt>
    <dd>
    <ul>
      <li>Programmes de surveillance, initiatives de sensibilisation à la sécurité publique, activités non répressives en soutien aux opérations, activités communautaires de financement et participation aux défilés et aux cérémonies publiques.</li>
      <li>Les auxiliaires de niveau&#160;1 exerceront ces activités sous la supervision générale d'un employé de la GRC et n'auront pas le statut d'agent de la paix.</li>
      <li>On s'attend à ce que les auxiliaires de niveau&#160;1 consacrent un minimum de 60&#160;heures par année au programme, ce qui comprend la formation. </li>
    </ul>
    </dd>
    <dt>Niveau&#160;2 </dt>
    <dd>
    <ul>
      <li>Les auxiliaires de niveau&#160;2 exercent toutes les activités prévues au niveau&#160;1 auxquelles s'ajoutent la présence dans la communauté (patrouilles à pied et à vélo), le contrôle d'accès et de la circulation et l'aide en cas de catastrophe.</li>
      <li>Les auxiliaires de niveau&#160;2 ne font pas de patrouilles opérationnelles.</li>
      <li>Les auxiliaires de niveau&#160;2 seront sous l'étroite supervision d'un membre régulier de la GRC.</li>
      <li>On s'attend à ce que les auxiliaires de niveau&#160;2 consacrent un minimum de 120&#160;heures par année au programme.</li>
    </ul>
    </dd>
    <dt>Niveau&#160;3 </dt>
    <dd>
    <ul>
      <li>Les auxiliaires du niveau&#160;3 exercent les activités prévues aux niveaux&#160;1 et&#160;2 en plus d'intervenir à la suite d'appels et de participer aux patrouilles générales, aux contrôles routiers, à la sécurité des lieux de crime et aux fouilles sur des personnes, à la demande d'un membre régulier.</li>
      <li>Les auxiliaires de niveau&#160;3 seront sous la supervision directe d'un membre régulier de la GRC.</li>
      <li>On s'attend à ce que les auxiliaires de niveau&#160;3 consacrent un minimum de 180&#160;heures par année au programme.</li>
    </ul>
    </dd>
  </dl>
  <h3>Formation</h3>
  <dl>
    <dt>Niveau&#160;1</dt>
    <dd>
      <ul>
        <li>Cours de la GRC en ligne qui portent sur des sujets comme la prévention du crime, la déontologie, l'éthique et l'orientation des auxiliaires. </li>
        <li>Cours externes&#160;: Secourisme général, RCR, défibrillateur externe automatisé (DEA) </li>
      </ul>
    </dd>
    <dt>Niveau&#160;2 </dt>
    <dd>
      <ul>
        <li>Les mêmes exigences que pour les auxiliaires de niveau&#160;1, auxquelles s'ajoutent des cours. </li>
      </ul>
    </dd>
    <dt>Niveau&#160;3 </dt>
    <dd>
      <ul>
        <li>Les mêmes exigences que pour les auxiliaires de niveaux&#160;1 et&#160;2, auxquelles s'ajoutent des cours. </li>
      </ul>
    </dd>
  </dl>
  <h3>Uniform</h3>
  <dl>
    <dt>Niveau&#160;1</dt>
    <dd>
      <ul>
        <li>Chandail polo gris </li>
        <li>Pantalon bleu sans bande porté avec une ceinture noire et des bottines noires </li>
        <li>Casquette de baseball bleu marine ou tuque bleu marine (facultatif) </li>
        <li>Blouson de patrouille jaune fluorescent 3&#160;en&#160;1 (facultatif) </li>
      </ul>
    </dd>
    <dt>Niveau&#160;2 </dt>
    <dd>
      <ul>
        <li>Chemise réglementaire grise </li>
        <li>Gilet haute visibilité porté par-dessus la chemise réglementaire en tout temps </li>
        <li>Pantalon bleu sans bande porté avec une ceinture de cuir noire et des bottines noires </li>
        <li>Casquette de baseball bleu marine ou tuque bleu marine </li>
        <li>Blouson de patrouille jaune fluorescent 3&#160;en&#160;1 </li>
        <li>Gilet pare-balles souple </li>
        <li>Gants réglementaires </li>
        <li>Gants résistant aux entailles </li>
        <li>Ceinturon de service et accessoires opérationnels </li>
        <li>Options d'intervention (ceinturon de service, bâton, menottes, etc.) </li>
        <li>Pantalon résistant aux intempéries </li>
        <!-- <li>Bicycle Shirts and Shorts (duty dependent)</li> -->
      </ul>
    </dd>
    <dt>Niveau&#160;3 </dt>
    <dd>
      <ul>
        <li>L'uniforme des auxiliaires de niveau&#160;3 est le même que celui des auxiliaires de niveau&#160;2. </li>
      </ul>
    </dd>
  </dl>
</section>
<section id="s5">
  <h2>Comment poser sa candidature </h2>
  <p>Pour plus d'information sur les qualifications requises, les niveaux de service et les modalités à suivre pour poser votre candidature, veuillez communiquer avec le bureau du Programme des auxiliaires dans votre province ou territoire. </p>
  <dl class="dl-horizontal">
    <dt>Alberta </dt>
    <dd><a href="kauxiliaryprogram-kprogrammeauxiliaire@rcmp-grc.gc.ca">kauxiliaryprogram-kprogrammeauxiliaire@rcmp-grc.gc.ca </a></dd>
    <dt>Colombie-Britannique </dt>
    <dd><a href="e.auxiliary.program-programme.auxiliaires.e@rcmp-grc.gc.ca">e.auxiliary.program-programme.auxiliaires.e@rcmp-grc.gc.ca </a></dd>
    <dt>Manitoba </dt>
    <dd><a href="rcmp.dauxiliaryprogram-dprogrammeauxiliaires.grc@rcmp-grc.gc.ca">rcmp.dauxiliaryprogram-dprogrammeauxiliaires.grc@rcmp-grc.gc.ca </a></dd>
    <dt>Nouveau-Brunswick </dt>
    <dd><a href="rcmp.jdivap-padivj.grc@rcmp-grc.gc.ca">rcmp.jdivap-padivj.grc@rcmp-grc.gc.ca </a></dd>
    <dt>Nouvelle-Écosse </dt>
    <dd><a href="hdiv_auxiliary-auxiliaires@rcmp-grc.gc.ca">hdiv_auxiliary-auxiliaires@rcmp-grc.gc.ca </a></dd>
    <dt>Saskatchewan </dt>
    <dd><a href="rcmp.fauxiliaryprogram-fprogrammeauxiliaire.grc@rcmp-grc.gc.ca">rcmp.fauxiliaryprogram-fprogrammeauxiliaire.grc@rcmp-grc.gc.ca </a></dd>
    <dt>Terre-Neuve-et-Labrador </dt>
    <dd><a href="rcmp.bdiv.auxprog-programme_des_auxiliaires.grc@rcmp-grc.gc.ca">rcmp.bdiv.auxprog-programme_des_auxiliaires.grc@rcmp-grc.gc.ca </a></dd>
    <dt>Territoires du Nord-Ouest </dt>
    <dd><a href="rcmp.gauxiliaryprogram-gprogrammeauxiliaire.grc@rcmp-grc.gc.ca">rcmp.gauxiliaryprogram-gprogrammeauxiliaire.grc@rcmp-grc.gc.ca </a></dd>
  </dl>
</section>
<section id="s6">
  <h2>Pour en savoir plus </h2>
  <ul>
    <li><a href="/en/bc/careers/volunteer-programs/bc-rcmp-auxiliary-program">La GRC cherche des candidats pour son nouveau Programme des auxiliaires en Colombie-Britannique</a></li>
  </ul>
</section>