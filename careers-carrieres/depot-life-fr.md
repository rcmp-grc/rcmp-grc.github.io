---
layout: careers
title: Aperçu de la vie à la Division Dépôt
date_modified: 2026-07-13
lang: fr
lang_url: depot-life-en.html
breadcrumbs:
  - label: "RCMP careers"
    url: "careers-carrieres/careers-carrieres-en.html"
  - label: "Police officer careers"
    url: "careers-carrieres/officer-agent-en.html"
custom_css: /assets/css/careers.css
---

<nav aria-labelledby="pagenav">
	<h2 id="pagenav">Sur cette page</h2>
	<ul>
		<li><a href="#s1">Découvrez la Division Dépôt</a></li>
		<li><a href="#s2">Cadet life overview</a></li>
		<li><a href="#s3">Living with a troop</a></li>
		<li><a href="#s4">Division Mess and other amenities</a></li>
		<li><a href="#s5">Support at Depot</a></li>
		<li><a href="#s6">Conduct and expectations for cadets</a></li>
		<li><a href="#s7">Community life in Regina</a></li>
		<li><a href="#s8">Related pages</a></li>
	</ul>
</nav>

<p>Bien que mentalement et physiquement exigeante, la vie à la Division Dépôt est profondément gratifiante. Vous serez poussés à vous dépasser, à vous adapter et à fonctionner dans un environnement structuré et rapide tout en développant discipline, confiance et résilience.</p>
<p>Vous tisserez des liens forts qui, souvent, se prolongeront durant toute votre carrière. Vous trouverez ci-dessous un aperçu de ce à quoi vous pouvez vous attendre durant votre formation de cadet à l’École de la GRC.</p>

<section id="s1">
	<h2>Découvrez la Division Dépôt</h2>
	<ul>
		<li>La Division Dépôt, où se trouve l’École de la GRC, est située à Regina (Saskatchewan).</li>
		<li>Il s’agit de la plus ancienne division de la GRC. Depuis 1885, elle a formé des générations de gendarmes à servir les communautés partout au Canada.</li>
		<li>Le campus de la Division Dépôt est connu à l’échelle nationale; il comprend des salles de classe, des installations de conditionnement physique et des espaces de formation tactique.</li>
		<li>C’est une installation de formation dynamique et sécurisée dont l’accès est contrôlé.</li>
		<li>Symbole de son enracinement dans la communauté, la Division Dépôt abrite le plus ancien bâtiment de Regina : la chapelle de la GRC.</li>
	</ul>
</section>

<section id="s2">
	<h2>Aperçu de la vie des cadets</h2>
	<ul>
		<li><strong>Les cadets sont affectés à une troupe de 32 membres</strong> aux origines diverses. with diverse backgrounds</li>
		<li><strong>Chaque troupe fonctionne comme un collectif</strong> : les membres prennent leurs repas ensemble, se déplacent sur la base en groupe et en uniforme, et cohabitent.: eating meals together, moving around base as a unit and dressing in uniform, and sharing accommodation</li>
		<li><strong>Le temps libre est limité durant la formation.</strong> La vie de cadet est faite de longues journées actives avec peu de temps libre. Life as a cadet involves long, active days with little time off<ul>
				<li>La formation se déroule généralement du lundi au vendredi. Les journées commencent tôt le matin et il y a des activités et des pratiques le soir et le week-end.</li>
				<li>Les jours fériés ne font pas exception, comme c’est le cas dans la vraie vie où les services de police fonctionnent 24 heures sur 24, 7 jours sur 7.</li>
			</ul>
		</li>
		<li><strong>La formation est exigeante sur le plan physique</strong> : l’entraînement permet de développer sa force et son endurance au fil du temps. Une bonne condition physique facilite la transition.: training builds strength and endurance over time. A good level of fitness helps ease the transition</li>
		<li><strong>Les cadets ont des rôles au sein de la troupe</strong>, tels que guide de droite, guide administratif, musicien dans l’orchestre de la Division Dépôt, agent de liaison, membres de comités et autres.: such as Right Marker, Admin Marker, playing in the Depot band, Unit Liaisons, Committee members, and other roles</li>
		<li><strong>La formation est donnée toute l’année</strong> : les conditions météorologiques varient en Saskatchewan. Préparez-vous à affronter la chaleur, le froid, le vent, la pluie, la neige et plus encore.: cadets experience the variable weather in Saskatchewan. Prepare for heat, cold, wind, rain, snow, and more.</li>
	</ul>
</section>

<section id="s3">
	<h2>La vie en troupe</h2>
	<p>Pendant les six mois (26 semaines) de leur formation, les cadets cohabitent avec leur troupe dans des dortoirs partagés. Apprendre à s’adapter à la vie en communauté et à rester organisé est un aspect important de la formation qui exige discipline et efficacité au quotidien.</p>
	<p>Les dortoirs sont équipés de ce qui suit :</p>
	<ul>
		<li>un espace cuisine avec réfrigérateur, micro-ondes, évier et rangement pour les denrées;</li>
		<li>un espace de détente pour la collaboration et les pauses<ul>
				<li>avec écrans télé, coin pour étudier ou se détendre et accès à un ordinateur protégé de la GRC;</li>
			</ul>
		</li>
		<li>des espaces de couchage individuels;</li>
		<li>des toilettes et des douches pour hommes, pour femmes et non genrées;</li>
		<li>un stand de tir à blanc, une salle de nettoyage des armes, une salle de repassage et une salle de cirage.</li>
	</ul>
	<h3>À quoi ressemble un espace de couchage</h3>
	<p>Chaque cadet dispose d’un petit espace de vie individuel, qui comprend :</p>
	<ul>
		<li>un lit à une place;</li>
		<li>un bureau et une chaise;</li>
		<li>un placard et des tiroirs;</li>
		<li>un rangement sécurisé;</li>
		<li>des accessoires, comme des bacs de rangement, des cintres, de la literie (couvertures, oreillers, etc.), une poubelle, un porte-stylo et une prise électrique;</li>
	</ul>
</section>

<section id="s4">
	<h2>Mess divisionnaire et autres commodités</h2>
	<dl>
		<dt>Mess de la Division (salle à manger)</dt>
		<dd>
			<ul>
				<li>Un menu varié et équilibré est proposé chaque semaine.</li>
				<li>La plupart des repas sont préparés à partir d’ingrédients frais.</li>
				<li>Le choix comprend des plats cuits à la vapeur, un comptoir à sandwichs, un buffet à salades et des produits de boulangerie.</li>
				<li>Les plats proposés conviennent à diverses diètes et préférences alimentaires : casher, halal, sans gluten, végétarien, végétalien et autres.</li>
			</ul>
		</dd>
		<dt>Recreational facilities</dt>
		<dd>These include outdoor fitness equipment, multiple gyms, a swimming pool, a disc golf course and an outdoor rink during the winter months.</dd>
		<dt>On-site parking</dt>
		<dd>Lots are accessible for private vehicles. There is no cost for parking. There are no plugins for block heaters for EVs in the cadet parking lot.</dd>
		<dt>On-site laundry</dt>
		<dd>Appliances operate with laundry cards for cadets to purchase and reload as needed.</dd>
	</dl>
</section>

<section id="s5">
	<h2>Support at Depot</h2>
	<p>Training at the RCMP Academy can be a daunting, but exciting experience. Cadets live away from family and friends for 6&nbsp;months, with new people, new challenges, a busy schedule, limited free time, and high expectations.</p>
	<p>Depot has a network of teams and services working together to create a healthy, respectful and safe training environment for cadets.</p>
	<ul>
		<li><strong>personal support</strong> includes a Cadet Resource Liaison, a Social Worker, Independent Centre for Harassment Resolution, chaplains and spiritual support, onsite doctor, nurses and a psychologist</li>
		<li><strong>spiritual spaces</strong> include the RCMP Chapel, the Indigenous Spirit Room and the Quiet Room</li>
		<li><strong>troop supports</strong> include facilitators, Sibling Troops, as well as wellness and diversity committees</li>
		<li><strong>the Learning Resource Center</strong> supports academic and professional excellence by providing quality library services and resources to cadets and employees</li>
	</ul>
</section>

<section id="s6">
	<h2>Conduct and expectations for cadets</h2>
	<dl>
		<dt>RCMP Core Values</dt>
		<dd>RCMP officers and cadets are expected to consistently live and demonstrate the <a href="/en/corporate-information/royal-canadian-mounted-police-core-values">RCMP’s core values</a>; take responsibility, show respect, serve with excellence, demonstrate compassion and act with integrity both on and off duty.</dd>
		<dt>Alcohol consumption</dt>
		<dd>Alcohol is not permitted in dorms or on balconies; licensed use is restricted to the Stand Easy Lounge on the base.</dd>
		<dt>Personal technology use</dt>
		<dd>Cadets can use personal devices with limited use during evenings and weekends. Devices are not allowed during exams.</dd>
		<dt>Schedule demands</dt>
		<dd>Time is limited, so connecting with family and friends may be difficult.</dd>
	</dl>
	</section>

	<section id="s7">
		<h2>Community life in Regina</h2>
		<p>Regina is Saskatchewan’s capital city. Home to Canadian Football League team, the Saskatchewan Roughriders, it also boasts one of the largest urban parks in North America and has over 500,000 planted trees. Bordered by dense woodlands, Depot is home to a wide range of wildlife. You will see a variety of birds, Canada geese, jackrabbits, ducks and more.</p>
		<p>There are many local amenities to explore including parks, walking and cycling paths, restaurants, movie theatres, malls, museums, places of worship, farmers’ markets, arts and entertainment. Essentials are easy to access, with many local grocery and retail stores.</p>
		<p>Transportation options include ride shares, cab companies, and public transit bus routes to explore the city. The Regina International Airport (YQR) is conveniently located just outside the city’s downtown hub, facilitating smooth transit in an out of the city.</p>
		<p>Cadets have opportunity to connect with the community by participating in local events hosted by Depot including the Depot Sunset Ceremonies, the RCMP National Memorial, local literacy nights, and other community parades and events.</p>
	</section>

	<section id="s8">
		<h2>Related pages</h2>
		<ul>
			<li><a href="/en/depot">RCMP Depot Division</a></li>
			<li><a href="/en/depot/virtual-tour">Virtual tour of Depot</a></li>
			<li><a href="/en/depot/training-tuesdays">Training Tuesdays</a></li>
			<li><a href="/en/depot/cadet-chronicles/week-0">Introduction: Welcome to Depot</a></li>
			<li><a href="/en/depot/services-and-information/events-depot-division">Events at Depot Division</a></li>
			<li><a href="/en/careers-rcmp/police-officer-careers/physical-standards-become-rcmp-officer">Physical standards to become an RCMP officer</a></li>
		</ul>
	</section>
