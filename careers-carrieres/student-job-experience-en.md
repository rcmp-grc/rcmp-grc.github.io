---
layout: careers
title: Student job experience offered at the RCMP
date_modified: 2026-08-31
lang: en
lang_url: student-job-experience-fr.html
breadcrumbs:
  - label: "RCMP careers"
    url: "careers-carrieres/careers-carrieres-en.html"
  - label: "Police officer careers"
    url: "careers-carrieres/officer-agent-en.html"
custom_css: /assets/css/careers.css
---

<p>We're actively recruiting students!</p>
<p>The RCMP offers unique job experiences in a wide variety of fields. Some student placement opportunities exist within the following areas:</p>
<dl>
	<dt>Corporate Management</dt>
	<dd>
		<ul>
			<li>procurement</li>
			<li>business management</li>
			<li>financial management</li>
			<li>business planning</li>
		</ul>
	</dd>
	<dt>Strategic Policy</dt>
	<dd>
		<ul>
			<li>public administration</li>
			<li>policies</li>
			<li>strategic planning</li>
		</ul>
	</dd>
	<dt>Communications</dt>
	<dd>
		<ul>
			<li>public affairs</li>
			<li>public relations</li>
			<li>communications strategist</li>
			<li>writer</li>
			<li>marketing</li>
			<li>advertising</li>
			<li>media relations</li>
			<li>digital communications</li>
			<li>web</li>
			<li>video</li>
			<li>social media</li>
		</ul>
	</dd>
	<dt>Technical Operations</dt>
	<dd>
		<ul>
			<li>cybercrime</li>
			<li>engineering</li>
			<li>security intelligence</li>
		</ul>
	</dd>
	<dt>Administrative Support</dt>
	<dd>
		<ul>
			<li>administrative assistant</li>
			<li>clerical</li>
			<li>executive assistant</li>
		</ul>
	</dd>
	<dt>Human Resources</dt>
	<dd>
		<ul>
			<li>labour relations</li>
			<li>classification</li>
			<li>staffing</li>
			<li>health services</li>
		</ul>
	</dd>
	<dt>Information Management and Information Technology Program (IM/IT)</dt>
	<dd>
		<ul>
			<li>computer-electronic based systems</li>
			<li>help desk</li>
			<li>radio services</li>
		</ul>
	</dd>
	<dt>Departmental Security</dt>
	<dd>
		<ul>
			<li>security management</li>
			<li>security services</li>
			<li>Information Technology (IT)</li>
			<li>communications security</li>
		</ul>
	</dd>
</dl>
