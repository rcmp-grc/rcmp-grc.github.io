---
layout: super_landing
title: Armes à feu
date_modified: 2026-05-11
lang: fr
lang_url: index.html
issued: 2026-05-05
hero: ip-cover-image-1200x726.jpg
intro: <p>Conformément à la <cite>Loi sur les armes à feu</cite>, le Programme canadien des armes à feu (<abbr>PCAF</abbr>) supervise les permis et l'enregistrement des armes à feu, maintient les normes nationales de formation de sécurité en matière d'armes à feu, vient en aide aux organismes d'application de la loi et a pour objectif de renforcer la sécurité publique. Les armes à feu au Canada demeurent réglementées par la Loi sur les armes à feu et la partie III du <cite>Code criminel</cite>.</p>
---

<div class="container">
  <section class="alert alert-warning mrgn-tp-md mrgn-bttm-md">
    <h2 class="h3">La Cour fédérale autorise le recours collectif contre le gouvernement du Canada pour atteinte à la vie privée</h2>
    <p>Si vous avez reçu un avis de certification ou si vous avez des raisons de croire que vous avez été victime d'une atteinte à la vie privée concernant vos renseignements personnels ou financiers consultés via des comptes en ligne du gouvernement du Canada entre le 1er mars et le 31 décembre 2020, vous pourriez être admissible à participer à un recours collectif&#160;: <a href="https://www.canada.ca/en/government/system/digital-government/online-security-privacy/sweet-hmk-class-action-suit.html">Government of Canada privacy breach class action</a>. </p>
  </section>
</div>
<section class="gc-most-requested">
  <div class="container">
    <h2>En demande</h2>
    <ul>
      <li>
        <a href="#">Services pour les particuliers</a>
      </li>
      <li>
        <a href="#">Services pour les entreprises</a>
      </li>
      <li>
        <a href="#">Services Web des agents de services publics</a>
      </li>
      <li>
        <a href="#">Tableau de référence des armes à feu</a>
      </li>
      <li>
        <a href="#">Formulaire de demande de permis pour les particuliers</a>
      </li>
      <li>
        <a href="#">Demande de permis pour entreprises, transporteurs, clubs et champs de tir</a>
      </li>
      <li>
        <a href="#">Instructeurs du Cours canadien de sécurité dans le maniement des armes à feu</a>
      </li>
    </ul>
  </div>
</section>
<div class="container mrgn-bttm-lg">
  <section class="gc-srvinfo">
    <h2 class="wb-inv">Services et information</h2>
    <div class="row wb-eqht-grd">
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Permis</a></h3>
        <p>Information sur les exigences relatives aux permis d'armes à feu, y compris les exceptions, pour les particuliers et les entreprises.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Au sujet des armes à feu</a></h3>
        <p>Au Canada, les armes à feu sont réparties en trois classes : armes sans restriction, armes à autorisation restreinte et armes prohibées.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Achat, vente et cession d'armes à feu</a></h3>
        <p>Information sur les exigences liées à l'achat, à la vente et au transfert d'armes à feu.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Importation et exportation</a></h3>
        <p>Information sur la procédure et les règlements applicables à l'importation et à l'exportation des armes à feu et des munitions et la liste des transporteurs autorisés.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Enregistrement, vérification et désactivation des armes à feu</a></h3>
        <p>Information sur les types d'armes à feu et les exigences liées à la vérification et l'enregistrement des armes à feu à autorisation restreinte et prohibées.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Transport et stockage des armes à feu</a></h3>
        <p>Information au sujet de la formation sur le maniement sécuritaire des armes à feu et lignes directrices sur les pratiques sécuritaires liées à l'entreposage, au transport et à l'exposition des armes à feu.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Sécurité et formation en matière d'armes à feu</a></h3>
        <p>Information au sujet de la formation sur le maniement sécuritaire des armes à feu et lignes directrices sur les pratiques sécuritaires liées à l'entreposage, au transport et à l'exposition des armes à feu.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Clubs et champs de tir</a></h3>
        <p>Information sur les lignes directrices de conception et les exigences sonores.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Armes à feu faisant partie d'une succession</a></h3>
        <p>Information à l'intention des exécuteurs testamentaires et des bénéficiaires d'armes à feu, y compris la procédure judiciaire qui s'applique.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Formulaires sur les armes à feu</a></h3>
        <p>Formulaires sur les armes à feu pour les particuliers, les entreprises, les policiers et les agents publics.</p>
      </div>
      <div class="col-lg-4 col-md-6">
        <h3><a href="#">Rapports sur les armes à feu</a></h3>
        <p>Rapports sur les armes à feu pour les particuliers, les entreprises, les policiers et les agents publics.</p>
      </div>
    </div>
  </section>
</div>
<div class="well well-sm brdr-0 mrgn-bttm-md">
  <div class="container">
    <section>
      <h2>Contactez-nous</h2>
      <ul class="wb-eqht list-unstyled mrgn-tp-lg lst-spcd-2 colcount-md-2 bold-content">
        <li>
          <a href="#">Programme canadien des armes à feu</a>
        </li>
        <li>
          <a href="#">Contrôleur des armes à feu</a>
        </li>
      </ul>
    </section>
  </div>
</div>
<div class="container">
  <div class="row">
    <div class="col-md-4">
      <section class="wb-feeds limit-3">
        <h2>Nouvelles</h2>
        <ul class="feeds-cont list-unstyled lst-spcd">
          <li>
            <a href="#">Rapport du <cite>Règlement sur le marquage des armes à feu</cite></a><br>
            <small class="feeds-date"><time>2025-11-30</time></small>
          </li>
          <li>
            <a href="#">Ne manquez pas les avis importants du Programme canadien des armes à feu</a><br>
            <small class="feeds-date"><time>2025-11-27</time></small>
          </li>
          <li>
            <a href="#">7 mars 2025, interdiction des armes à feu</a><br>
            <small class="feeds-date"><time>2025-10-17</time></small>
          </li>
        </ul>
        <p>Plus : <a href="#">Nouvelles d'armes à feu</a></p>
      </section>
    </div>
    <div class="col-md-8">
      <section class="gc-features">
        <h2>En vedette</h2>
        <div class="row wb-eqht-grd">
          <div class="col-sm-6 hght-inhrt">
            <div class="well well-sm eqht-trgt hght-inhrt">
              <img alt="" src="feature-1-360x203.png">
              <h3><a class="stretched-link" href="#">[Feature hyperlink text]</a></h3>
              <p>Brief description of the feature being promoted. Brief description of the feature being promoted.</p>
            </div>
          </div>
          <div class="col-sm-6 hght-inhrt">
            <div class="well well-sm eqht-trgt hght-inhrt">
              <img alt="" src="feature-2-360x203.png">
              <h3><a class="stretched-link" href="#">[Feature hyperlink text]</a></h3>
              <p>Brief description of the feature being promoted. Brief description of the feature being promoted.</p>
            </div>
          </div>
        </div>
      </section>
    </div>
  </div>
</div>
